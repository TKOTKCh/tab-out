 ...
 * Includes chrome:///edge:// pages like extensions which can be safely closed.
 */
async function closeAllDuplicateTabs() {
  const allTabs = await chrome.tabs.query({});
  
  // Filter out only pages that shouldn't be closed programmatically
  // Allow chrome:///edge:// pages like extensions, history, downloads to be closed
  const realTabs = allTabs.filter(t => {
    const url = t.url || '';
    // Skip Tab Out pages and browser startup pages
    if (url === 'chrome://newtab/' || url === 'edge://newtab/' || url.startsWith('chrome-extension://')) {
      return false;
    }
    ...
    // Allow chrome:// and edge:// pages (extensions, history, downloads, etc.)
    return true;
  });
...
async function getBrowserHistory(query = '', limit = 20) {
  try {
    ...
    for (const item of historyItems) {
      if (!seenUrls.has(item.url) && item.url && !item.url.startsWith('chrome://') && !item.url.startsWith('edge://') && !item.url.startsWith('chrome-extension://')) {
  ...
  html += `
    <div class="search-result-item" data-query="${encodeURIComponent(query)}" data-action="web-search">
      ...
      <div class="search-result-content">
        ...
        <div class="search-result-url">https://www.bing.com/search?q=${encodeURIComponent(query)}</div>
      </div>
